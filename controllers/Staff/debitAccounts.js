...
logic.debitAccount = (req, res) => {
  ...
  if (!req.params.account_number) return services.errorResponse(res, 400, 'Account number is required');
  ...
  if (!services.checkNumber(req.headers['cashier-id'])) return services.errorResponse(res, 400, 'Cahsier id must be numbers');
  ...
  if (!findAccountNumber) return services.errorResponse(res, 400, 'Account number not found');
  if (!verifyCashier) return services.errorResponse(res, 400, 'Only registered staff can debit or credit an bank account');
